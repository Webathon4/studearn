...
const AuctionSettlementChat = () => {
  ...
  const loadChat = async () => {
    try {
      const res = await api.get(`/api/auction-settlement-chats/${auctionId}`);
      setChat(res.data);
    } catch (err) {
      setError(err.response?.data?.message || "Failed to load settlement chat");
    }
  };
  ...
  const handleSend = async (event) => {
    ...
    setError("");
    try {
      const res = await api.post(`/api/auction-settlement-chats/${auctionId}/messages`, {
        text: message,
      });
      setChat(res.data);
      setMessage("");
    ...
    <div className="container">
      <div className="page-header">
        <h1 className="page-title">
          ...
          Auction Settlement Chat
        </h1>
        <p className="page-description">Coordinate payment and pickup details</p>
      </div>
        ...
        <div className="chat-window">
          {chat?.messages?.length ? (
            chat.messages.map((msg, index) => {
              ...
              const sellerId = normalizeId(chat.sellerId);
      ...
      {chat && !chat.isActive && (
        <p className="warning alert alert-warning">Chat closed. Settlement completed.</p>
      )}
